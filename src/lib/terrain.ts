...
export const ISLAND_RADIUS = 30 // dome reaches 0 height around r≈23.6
...
export function terrainHeight(x: number, z: number): number {
  ...
  const dome = Math.pow(Math.max(0, 1 - r * r), 1.6) * 4.2 - 0.9
  // the dune the hero palm stands on
  const dune = Math.exp(-((x - 7) ** 2 + (z + 5) ** 2) / 18) * 1.5
  // gentle rolling variation, fading out toward the water
  const n = (fbm(x * 0.09 + 3.7, z * 0.09 - 1.3) - 0.5) * 1.7 * Math.max(0, 1 - r)
